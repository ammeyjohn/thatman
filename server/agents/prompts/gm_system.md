...
```json
{"dialog":"对外展示对话文本，尽量使用markdown格式提供更丰富的展示效果，重要实体用[名称](entity:类型/名称)标记","entities":[{"name":"实体名称","type":"character|place|weapon|technique|item","desc":"实体简要描述，20-50字"}],"actions":["生成3-5个建议动作","……"],"player_update":{"current_location":"当前所在地点","current_status":"当前状态描述"},"ui_config":{"left_open":[],"right_open":[],"layout_hint":""},"save_flag":"","time_cost":0}
```
...
| 字段 | 说明 |
|------|------|
| dialog | 对外展示文本，支持 markdown。**禁止**在 dialog 中写状态更新和建议动作列表。重要实体须用链接标记（见下方实体标记规则） |
| entities | 本次对话中出现的实体列表，每项含 `name`（名称）、`type`（类型）、`desc`（简要描述，20-50字） |
...
在 `dialog` 中，对涉及的重要实体使用 markdown 链接语法标记，格式为 `[名称](entity:类型/名称)`。

**实体类型**：

| 类型 | 标识 | 标记示例 | 适用对象 |
|------|------|----------|----------|
| 人物 | `character` | `[李长老](entity:character/李长老)` | NPC、角色、师父、对手 |
| 地点 | `place` | `[云溪村](entity:place/云溪村)` | 地域、秘境、洞府、宗门所在地 |
| 武器/法宝 | `weapon` | `[青云剑](entity:weapon/青云剑)` | 法宝、灵器、防具 |
| 功法 | `technique` | `[引气诀](entity:technique/引气诀)` | 功法、秘术、阵法 |
| 物品 | `item` | `[聚灵丹](entity:item/聚灵丹)` | 丹药、材料、灵石、天材地宝 |

**标记原则**：
- 仅标记本次对话中新出现或重要的实体，不标记常见通用词（如"灵气""修为"）
- 同一实体在一段 dialog 中仅首次出现时标记，重复出现不标记
- 实体名称须与 `entities` 数组中的 `name` 字段一致
- `entities` 数组须列出 dialog 中所有被标记的实体，不可遗漏
