...
可拜师、结仇、隶属宗门、产生羁绊关系。
7. ❌ 语音输入系统
支持玩家通过语音输入与AI GM交互，语音自动转文字送入对话流程。
8. ❌ 语音输出系统
支持AI GM回复内容通过语音播报输出，提供沉浸式修仙听觉体验。

### 三、角色修仙养成体系 🟡 部分完成
1. ✅ 境界修炼突破系统（境界体系配置、属性成长规则、AI动态判定突破）
2. ✅ 打坐耗时修炼系统（支持基础打坐、深度修炼、突破闭关，含功法/丹药/环境/灵气多因素耗时修正）
3. ✅ 灵力运转、境界压制机制（境界压制规则已配置）
4. ✅ 寿元增减系统（寿元字段与境界寿命规则已配置）
5. ❌ 心魔、悟道、感悟机制（仅配置文档定义，无独立判定代码，依赖LLM GM叙事）
6. ✅ 功法习得、升级、遗忘系统（功法配置与角色功法字段已配置）
7. ❌ 武技、秘术修炼系统（仅配置文档与通用修炼动作定义，无武技/秘术专属交互代码）
8. ✅ 法宝获得、佩戴、替换系统（装备字段与装备面板已配置）
9. ❌ 法宝淬炼、炼化、进阶系统
10. ✅ 丹药服用、增益、药效消退系统（物品配置与背包系统已配置）
11. ❌ 炼丹系统（仅配置文档与动作定义，无配方数据结构与产出逻辑，依赖LLM GM叙事）
12. ❌ 炼器系统（仅配置文档与动作定义，无配方数据结构与产出逻辑，依赖LLM GM叙事）

### 四、地图探索大世界玩法 🟡 部分完成
1. ✅ 大地图区域穿梭、赶路系统（地域配置与坐标系统已配置）
2. ✅ 秘境刷新、限时秘境系统（秘境配置已配置，AI动态生成秘境事件）
3. ✅ 禁地、危险区域、特殊地貌机制（环境增益/负面效果规则已配置）
4. ❌ 野外妖兽刷新、探索遭遇系统（仅配置文档定义，无妖兽刷新与遭遇判定代码，依赖LLM GM叙事）
5. ✅ 资源采集（灵草、矿石、灵药）（材料物品配置已配置）

### 五、宗门势力玩法 🟡 部分完成
1. ✅ 加入宗门、脱离宗门机制（宗门配置与角色宗门字段已配置）
2. ✅ 宗门职位、权限体系（宗门职位规则已配置）
3. ✅ 宗门任务、宗门贡献系统（任务配置已配置）
4. ❌ 宗门俸禄、宝库、福利系统（仅配置文档定义俸禄表，无发放与访问代码）
5. ❌ 宗门大比、宗门考核
6. ✅ 宗门兴衰、扩张、覆灭机制（世界定时推演已包含宗门兴衰）
7. ❌ 自建宗门、开山立派玩法（创建条件配置已定义，实际代码待实现）

### 六、NPC深度AI互动 🟡 部分完成
1. ✅ NPC自主生活、作息、走动AI（世界定时推演已包含NPC自主行事）
2. ❌ NPC好感度、亲密度系统
3. ✅ NPC送礼、交友、结怨系统（关系存储系统已配置，AI动态生成关系变更）
4. ❌ 完整主线、支线任务链系统（仅配置文档定义任务类型与奖励，无任务链追踪与状态管理代码）
5. ❌ NPC传道、授业、指点机缘（仅配置文档定义传道规则，无交互代码，依赖LLM GM叙事）

### 七、战斗斗法系统 🟡 部分完成
1. 🟡 文字即时战斗系统（CombatManager已实现PvP切磋流程与LLM仲裁，缺PvE战斗与数值化引擎）
2. ❌ 功法对决、法宝对战机制
3. ❌ 闪避、防御、反击、遁逃机制（仅配置文档定义身法加成，无独立判定代码，依赖LLM GM叙事）
4. ❌ 阵法对战、多人合击系统（阵法体系配置已定义，对战实现待开发）
5. ❌ BOSS秘境讨伐战斗（仅配置文档提及秘境与BOSS，无BOSS战机制代码）

### 八、经济交易体系 🟡 部分完成
1. ✅ 灵石货币体系（物品配置已包含灵石，AI动态处理货币交易）
2. ❌ 坊市商店购买机制（仅配置文档定义交易权限与定价，无商店交互代码，依赖LLM GM叙事）
3. ❌ 玩家/NPC摆摊交易
4. ❌ 拍卖行竞价系统
5. ❌ 材料合成、炼器、炼丹玩法（仅配置文档与动作定义，无合成配方与产出逻辑，依赖LLM GM叙事）
...
### 十一、多人社交玩法 🔄 开发中
1. ✅ 多玩家在线共存
   - OnlineManager 内存在线状态追踪，60秒心跳超时自动离线
   - EventBus 支持按 uid 定向推送和区域频道订阅（area:位置）
   - SSE 连接绑定 uid，推送上线/下线/位置变化等事件
   - 附近人物接口包含同区域在线真实玩家（type=player）
2. ✅ 玩家组队、结伴修行
   - TeamManager 管理队伍创建/邀请/踢出/队长转移/解散
   - 队伍数据持久化至 CouchDB game_teams
3. ✅ 玩家切磋、斗法PK
   - CombatManager 非实时回合制切磋，AI GM 仲裁战斗
   - 双方提交行动后 LLM 生成战斗描述与结果
   - 支持挑战/接受/行动/逃跑完整流程
4. ✅ 玩家结为道侣、师徒
   - SocialManager 管理好友/师徒/道侣三种社交关系
   - 关系数据存储于 game_links，link_type 区分 friend/master_disciple/dao_companion
   - 支持申请/接受/拒绝/解除完整流程
   - 私聊与区域聊天：ChatManager + EventBus 定向/频道推送
5. ❌ 玩家自建势力、结盟对抗
